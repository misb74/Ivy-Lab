...
Work with Airtable's REST API directly. No MCP server, no OAuth flow, no Python SDK — just a personal access token.

> **Lab note:** `.claude/settings.json` deny-lists raw `curl`. The curl examples below will hit a permission prompt when invoked in Lab. Two paths forward in this environment:
> 1. **Approve the prompt** ad-hoc when running the curl commands (simplest).
> 2. **Build a stdlib Python wrapper** following the pattern in `.claude/skills/linear/scripts/linear_api.py` if you'll use Airtable repeatedly (cleaner for agent loops).
>
> Wrapper is not pre-built because the Linear-vs-Airtable choice for engagement state is unresolved at install time — build the wrapper only for whichever one you commit to. Decision tracked in `~/.ivy-lab/consulting-stack.md`.
