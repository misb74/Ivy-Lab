...
Manage Linear issues, projects, and teams directly via the GraphQL API. No MCP server, no OAuth flow, no extra dependencies.

> **Lab note:** `.claude/settings.json` deny-lists raw `curl`. **In Ivy-Lab, always use `scripts/linear_api.py`** (stdlib `urllib` — no curl). The curl examples below are documented for reference but will hit a permission prompt when invoked in Lab. The Python helper covers the full surface: `whoami`, `list-teams`, `list-projects`, `list-issues`, `get-issue`, `search-issues`, `create-issue`, `update-issue`, `update-status`, `add-comment`. Run `python .claude/skills/linear/scripts/linear_api.py --help` for usage.
