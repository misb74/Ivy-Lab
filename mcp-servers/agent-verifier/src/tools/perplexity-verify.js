...
async function querySonar(systemPrompt, userPrompt, options) {
    ...
    const citations = (data.citations || []).map((c) => typeof c === 'string' ? c : c.url);
...
export async function adversarialReview(params) {
    ...
    if (checks.directional_framing) {
        checkInstructions.push(`- directional_framing: The report says "${checks.directional_framing.report_says}". Check whether the source actually says this, or says the inverse/opposite. Pass = framing matches source. Fail = inverted or misframed.`);
    }
    if (checks.source_primacy) {
        checkInstructions.push(`- source_primacy: Classify "${source}" as one of: primary_dataset, peer_reviewed, industry_survey, derivative_commentary, aggregated_estimate, or unverifiable. Pass = primary_dataset, peer_reviewed, or industry_survey. Fail = derivative_commentary, aggregated_estimate, or unverifiable.`);
    }
    if (checks.attribution) {
        const { person, title, organization } = checks.attribution;
        checkInstructions.push(`- attribution: Does ${person} currently hold the title "${title}" at ${organization}? Pass = still in role. Fail = departed or title changed. Include departure date if known.`);
    }
    if (checks.scope_match) {
        checkInstructions.push(`- scope_match: The source's stated scope is: "${checks.scope_match.source_scope}". The report uses this figure as: "${checks.scope_match.report_usage}". Does the source scope match how the report uses it? Pass = matches. Fail = broader_than_source, narrower_than_source, or different_scope.`);
    }
...
SOURCE DATE: ${source_date}`;
    ...
    const isHighRisk = (framingFailed && framingNoteInverted) ||
...
export async function findLatest(params) {
...
//# sourceMappingURL=perplexity-verify.js.map