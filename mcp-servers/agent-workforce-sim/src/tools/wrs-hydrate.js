...
import { buildMockConnectorExecutor, hydrateRoleWithPolicy, } from '../hydrators/role-hydrator.js';
/**
...
export async function handleWrsHydrate(input) {
    ...
    const role_rows = db
      ...
      `)
        .all(input.simulation_id, input.role_id ?? null, input.role_id ?? null);
    ...
    if (input.prefetched_sources !== undefined) {
        ...
        throw new Error('wrs_hydrate requires either prefetched_sources or use_mock_data=true. ' +
            'Pass prefetched_sources from the gateway orchestrator, or set use_mock_data=true for development.');
    }
    const degraded_source_set = new Set();
    ...
    for (const role of role_rows) {
      ...
      WHERE id = ?
    `).run(hydrated.automation_potential, hydrated.worker_desire_avg, hydrated.aei_exposure_score, hydrated.felten_aioe_score, hydrated.human_edge_avg, hydrated.annual_cost_per_fte, role.role_id);
    ...
    `);
        for (const task of hydrated.tasks) {
            ...
            const linkedSkills = provenance[taskHash]
                ?? Object.entries(provenance).find(([_, skills]) => skills.some(s => task.task_statement.toLowerCase().includes(s.skill_name.toLowerCase().slice(0, 10))))?.[1]
                ?? [];
            const linkedSkillsJson = linkedSkills.length > 0
                ? JSON.stringify(linkedSkills.map(s => s.skill_name))
                : null;
            task_insert.run(crypto.randomUUID(), role.role_id, task.onet_task_id, task.task_statement, task.importance, task.time_allocation, task.ai_capability_score, task.worker_desire_score, task.human_agency_scale, task.aei_penetration_rate, task.aei_autonomy, task.aei_collaboration_pattern, task.human_edge_stakeholder_trust, task.human_edge_social_intelligence, task.human_edge_creativity, task.human_edge_ethics, task.human_edge_physical_presence, task.human_edge_judgment, linkedSkillsJson, now);
    ...
    `);
        for (const skill of hydrated.skills) {
            skill_insert.run(crypto.randomUUID(), role.role_id, skill.skill_name, skill.lightcast_skill_id, skill.level, skill.importance, skill.trend, now);
        }
    ...
    WHERE id = ?
  `).run('hydrated', JSON.stringify(Array.from(degraded_source_set)), used_mock_data ? 1 : 0, now, input.simulation_id);
    return {
...
//# sourceMappingURL=wrs-hydrate.js.map