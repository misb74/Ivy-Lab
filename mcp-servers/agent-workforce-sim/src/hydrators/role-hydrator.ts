...
export interface HydratedRoleResult {
  ...
  task_skill_provenance: Record<string, Array<{ skill_id: string; skill_name: string; similarity: number }>>;
...
export type HydratorExecutor = (
  toolName: string,
  input: Record<string, unknown>
) => Promise<unknown>;

export const CONNECTOR_RELIABILITY_POLICY: ConnectorPolicy[] = [
  {
...
export async function hydrateRoleWithPolicy(input: HydratorInput): Promise<HydratedRoleResult> {
  const execute = input.execute ?? buildUnavailableExecutor();

  const payloads = buildPayloads(input);
  const source_status: HydratedRoleResult['source_status'] = [];
  const source_results: Record<string, unknown> = {};
  const degraded_sources: string[] = [];
  const required_failures: string[] = [];
  ...
  const settled = await Promise.allSettled(
    base_policies.map(async (policy) => {
      const attemptResult = await runWithRetries(
        () => runWithTimeout(execute(policy.tool_name, payloads[policy.tool_name] ?? {}), policy.timeout_ms, policy.tool_name),
        policy.retries
      );
  ...
  const human_edge_collection = edge_policy
    ...
    : { signals: new Map<string, HumanEdgeSignal>(), success_count: 0, total_count: 0 };
...
function mergeTaskSignals(
  ...
  human_edge_by_statement: Map<string, HumanEdgeSignal>
): HydratedRoleTask[] {
...
function buildTaskSeeds(source_results: Record<string, unknown>): TaskSeed[] {
...
function buildTaskSignalStore(source_results: Record<string, unknown>): {
  by_id: Map<number, TaskSignals>;
  by_statement: Map<string, TaskSignals>;
} {
  const by_id = new Map<number, TaskSignals>();
  const by_statement = new Map<string, TaskSignals>();

  const upsert = (row: Record<string, unknown>, patch: TaskSignals) => {
  ...
  for (const row of extractAeiRows(source_results['aei_task_collaboration'])) {
    upsert(row, {
      aei_collaboration_pattern: String(
        row.collaboration_pattern ?? row.collaboration ?? row.pattern ?? 'human-in-loop'
      ),
      aei_autonomy: asNumber(row.autonomy ?? row.ai_autonomy),
    });
  }
...
function resolveSignals(
  signal_store: { by_id: Map<number, TaskSignals>; by_statement: Map<string, TaskSignals> },
  seed: TaskSeed
): TaskSignals {
...
async function collectHumanEdgeSignals(input: {
  ...
  signals: Map<string, HumanEdgeSignal>;
  success_count: number;
  total_count: number;
}> {
  const by_statement = new Map<string, HumanEdgeSignal>();
  const settled = await Promise.allSettled(
    input.tasks.map(async (task) => {
      const result = await runWithRetries(
        () =>
          runWithTimeout(
            input.execute('workbank_human_edge', { task_statement: task.task_statement }),
            input.policy.timeout_ms,
            'workbank_human_edge'
          ),
        input.policy.retries
      );
      return {
...
function parseHumanEdgeSignal(source: unknown): HumanEdgeSignal | null {
...
function deriveAeiExposure(source_results: Record<string, unknown>, tasks: HydratedRoleTask[]): number {
...
function deriveFeltenExposure(source_results: Record<string, unknown>): number {
...
function deriveAnnualCost(source_results: Record<string, unknown>): number {
  const payload = asObject(unwrapToolResult(source_results['bls_occupation_wages']));
  const annual = asNumber(
    payload?.median_annual_wage
...
const SKILL_NOISE_FILTERS: Array<{ exclude_soc_prefixes: string[]; blocked_skills: Set<string> }> = [
  {
...
function isNoiseSkill(skillName: string, socCode: string): boolean {
  const lower = skillName.toLowerCase().trim();
  for (const filter of SKILL_NOISE_FILTERS) {
    if (filter.exclude_soc_prefixes.some((p) => socCode.startsWith(p))) {
      if (filter.blocked_skills.has(lower)) return true;
    }
  }
  return false;
}

function getTaskSkillProvenance(
  source_results: Record<string, unknown>,
): Record<string, Array<{ skill_id: string; skill_name: string; similarity: number }>> {
  ...
  if (provenance && typeof provenance === 'object' && !Array.isArray(provenance)) {
    return provenance as Record<string, Array<{ skill_id: string; skill_name: string; similarity: number }>>;
  }
  return {};
}

function getSkillRecords(source_results: Record<string, unknown>, onet_soc_code?: string): HydratedRoleSkill[] {
  const skill_map = new Map<string, HydratedRoleSkill>();
  const register = (skill_name: string, partial?: Partial<HydratedRoleSkill>) => {
  ...
  for (const row of atlas_rows) {
    ...
    const lightcastSkillId = (
      category === 'lightcast'
      || skillId.startsWith('KS')
      || skillId.startsWith('ES')
      || skillId.startsWith('BGS')
    ) ? skillId || null : null;
...
function getRoleSpecificFallbackSkills(roleName: string): string[] {
  const profileMap: Array<{ match: RegExp; skills: string[] }> = [
    {
...
function extractWorkbankRows(source: unknown): Array<Record<string, unknown>> {
  const payload = unwrapToolResult(source);
  const object_payload = asObject(payload);
  if (Array.isArray(payload)) {
    return payload.filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
  }
  if (!object_payload) return [];
  const candidates = asArray(object_payload.tasks)
    ?? asArray(object_payload.data)
    ?? [];
  return candidates.filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
}

function extractGapRows(source: unknown): Array<Record<string, unknown>> {
  const payload = asObject(unwrapToolResult(source));
  if (!payload) return [];
  const arrays = [
  ...
  return arrays.flat().filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
}

function extractAeiRows(source: unknown): Array<Record<string, unknown>> {
  const payload = unwrapToolResult(source);
  if (Array.isArray(payload)) {
    return payload.filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
  }
  ...
  return rows.filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
}

function extractSkillRows(source: unknown): Array<Record<string, unknown>> {
  if (Array.isArray(source)) {
    return source.filter((row): row is Record<string, unknown> => Boolean(asObject(row)));
  }
...
function unwrapToolResult(source: unknown): unknown {
...
function parseTaskId(value: unknown, fallback_index: number): number {
  const numeric = Number(value);
  if (Number.isFinite(numeric) && numeric > 0) {
    return Math.round(numeric);
  }
  const text = String(value ?? '').trim();
  if (!text) return fallback_index + 1;
...
function normalizeTrend(value: unknown): number {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 0;
  if (numeric >= -1 && numeric <= 1) return numeric;
  if (numeric > 1 && numeric <= 5) return (numeric - 3) / 2;
  return clampSigned(numeric / 100, -1, 1);
}

function normalizeUnitScale(value: unknown, fallback: number): number {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return clamp01(fallback);
  if (numeric >= 0 && numeric <= 1) return numeric;
  if (numeric > 1 && numeric <= 5) return clamp01(numeric / 5);
  if (numeric > 5 && numeric <= 7) return clamp01(numeric / 7);
  if (numeric > 7 && numeric <= 100) return clamp01(numeric / 100);
...
async function runWithRetries(fn: () => Promise<unknown>, retries: number): Promise<AttemptResult> {
  let attempt = 0;
...
function runWithTimeout<T>(promise: Promise<T>, timeout_ms: number, tool_name: string): Promise<T> {
...
function buildPayloads(input: HydratorInput): Record<string, Record<string, unknown>> {
  return {
...
function buildUnavailableExecutor(): HydratorExecutor {
  return async (_toolName: string) => {
    throw new Error('No connector executor configured for hydrator');
  };
}

export function buildMockConnectorExecutor(): HydratorExecutor {
  return async (toolName: string, payload: Record<string, unknown>) => {
    ...
    const SOC_PROFILES: Record<string, MockRoleProfile> = {
      '43-3031': {
        tasks: [
    ...
    const TITLE_PROFILES: Array<{ match: RegExp; profile: MockRoleProfile }> = [
      // ── Finance ─────────────────────────────────
    ...
    const caps = (profile?.caps ?? [0.5, 0.45, 0.38, 0.34]).map((value, idx) =>
      clamp01(value + ((v - 0.5) * 0.15) + idx * 0.008)
    );
    const desires = (profile?.desires ?? [0.5, 0.46, 0.4, 0.34]).map((value, idx) =>
      clamp01(value + ((v - 0.5) * 0.12) - idx * 0.006)
    );
    const trusts = (profile?.trust ?? [0.35, 0.4, 0.5, 0.6]).map((value, idx) =>
      clamp01(value + ((v - 0.5) * 0.1) + idx * 0.005)
    );
    const socials = (profile?.social ?? [0.3, 0.35, 0.42, 0.5]).map((value, idx) =>
      clamp01(value + ((v - 0.5) * 0.08) + idx * 0.006)
    );
    ...
    if (toolName === 'lightcast_search_skills') {
      const skillsByRole: Array<{ match: RegExp; skills: string[] }> = [
        // ── Finance ──
